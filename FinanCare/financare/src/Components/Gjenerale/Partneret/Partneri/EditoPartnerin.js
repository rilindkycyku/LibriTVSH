...
function EditoKompanin(props) {
  ...
  useEffect(() => {
    const shfaqpartneret = async () => {
      try {
        const partneri = data.shfaqPartneret.filter(
          (item) => item.idPartneri == props.id
        );
        setPartneri(partneri[0]);
        setLlojiPartnerit(partneri[0].llojiPartnerit);
        setRabati(partneri[0]?.kartela?.rabati ?? 0);
      } catch (err) {
        console.log(err);
      }
    };
