...
function ShtoPerdorues(props) {
  ...
  const [nrLlogarisBankare, setNrLlogarisBankare] =
    useState("1290012345678900");
  ...
  useEffect(() => {
    const fetchedoptionsa = data.shfaqBankat.map((item) => ({
      value: item.bankaID,
      label: item.emriBankes,
    }));
    setOptions(fetchedoptionsa);

    const fetchedoptions = data.shfaqRolet
