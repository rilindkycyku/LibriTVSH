...
function EditoLlogarin(props) {
  ...
  useEffect(() => {
        const fetchedoptions = data.shfaqBankat.map((item) => ({
          value: item.bankaID,
  ...
  useEffect(() => {
    const shfaqNjesineMatese = async () => {
      try {
        const bankaKerkim = data.ShfaqLlogaritEBiznesit.filter((item) => item.idLlogariaBankare == props.id);
        setOptionsSelected(
          options.filter((item) => item.value == bankaKerkim[0].bankaID)
        );
        setBanka(bankaKerkim[0]);
      } catch (err) {
        console.log(err);
      }
    };
