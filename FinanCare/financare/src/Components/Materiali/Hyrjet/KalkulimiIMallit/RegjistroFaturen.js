...
function RegjistroFaturen(props) {
    ...
    if (props.idKalkulimitEdit != 0) {
      const vendosTeDhenat = async () => {
        try {
          ...
          const teDhenatFatures = data.shfaqRegjistrimetNgaID39;
  ...
  async function handleEdit(id, index) {
    ...
    setQmimiShitesMeShumic(p[0]?.qmimiShitesMeShumic);
  }
  ...
  useEffect(() => {
    const fetchedoptions = data.ProduktetPerKalkulim.map((item) => ({
