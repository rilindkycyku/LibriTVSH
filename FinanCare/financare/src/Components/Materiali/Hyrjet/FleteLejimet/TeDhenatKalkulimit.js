...
function TeDhenatKalkulimit(props) {
  ...
  useEffect(() => {
    const vendosTeDhenat = async () => {
      try {
        setLoading(true);
        const produktet = data.shfaqTeDhenatKalkulimit.filter(
          (item) => item.idRegjistrimit == props.id
        );
  ...
  useEffect(() => {
    const shfaqTeDhenatFature = async () => {
      try {
        setLoading(true);
        const teDhenat = data.shfaqRegjistrimetNgaID;
              ...
              <Container fluid>
                ...
                <Row>
                  <Col className={classes.mobileResponsive}>
                    <h4>Partneri: {teDhenatFat?.regjistrimet?.emriBiznesit}</h4>
                    <h4>Nr. Fatures: {teDhenatFat?.regjistrimet?.nrFatures}</h4>
                    <h4>
                      Data Fatures:{" "}
                      {new Date(
                    ...
                    <h4>
                      Totali Me TVSH:{" "}
                      {parseFloat(
                        teDhenatFat.tvsh + teDhenatFat.totaliPaTVSH
                      ).toFixed(2)}{" "}
                      €
                    </h4>
                  </Col>
                  <Col className={classes.mobileResponsive}>
                    ...
                    <p>
                      ...
                      {teDhenatFat?.regjistrimet?.llojiPageses}
                    </p>
                    <p>
                      <strong>Statusi i Pageses:</strong>{" "}
                      {teDhenatFat?.regjistrimet?.statusiPageses}
                    </p>
                  </Col>
                  <Col className={classes.mobileResponsive}>
                    <p>
                      <strong>Personi Pergjegjes:</strong>{" "}
                      {teDhenatFat?.regjistrimet?.stafiID + " - " + teDhenatFat?.regjistrimet?.username}
                    </p>
                    <p>
