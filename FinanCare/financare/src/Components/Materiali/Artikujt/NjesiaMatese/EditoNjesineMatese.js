...
function EditoNjesineMatese(props) {
  ...
  useEffect(() => {
    const vendosNjesiteMatese = async () => {
      try {
        setNjesiteMatese(data.shfaqNjesiteMatese);
      } catch (err) {
        console.log(err);
      }
    };
  ...
  useEffect(() => {
    const shfaqNjesineMatese = async () => {
      try {
        const njesiaMatese = data.shfaqNjesiteMatese.filter(
          (item) => item.idNjesiaMatese == props.id
        );
        setNjesiaMatese(njesiaMatese[0]);
      } catch (err) {
        console.log(err);
      }
    };
  ...
  const handleNjesiaMateseChange = (value) => {
    setNjesiaMatese((prev) => ({ ...prev, njesiaMatese: value }));
  };
  ...
  const handleKontrolli = () => {
    if (isNullOrEmpty(njesiaMatese.njesiaMatese)) {
          ...
          <Form>
            ...
            <Form.Group className="mb-3" controlId="exampleForm.ControlInput1">
              ...
              <Form.Control
                ...
                value={njesiaMatese.njesiaMatese}
                type="text"
