...
function EditoGrupetEProduktit(props) {
  ...
  useEffect(() => {
    const vendosNjesiteMatese = async () => {
      try {
        setNjesiteMatese(data.shfaqGrupetEProduktit);
      } catch (err) {
        console.log(err);
      }
    };
  ...
  useEffect(() => {
    const shfaqNjesineMatese = async () => {
      try {
        const gr = data.shfaqGrupetEProduktit.filter(
          (item) => item.idGrupiProduktit == props.id
        );
        setNjesiaMatese(gr[0]);
      } catch (err) {
        console.log(err);
      }
    };
  ...
  function handleSubmit() {
    ...
    props.setPershkrimiMesazhit("Grupi i Produktit u Perditesua me sukses!");
