...
function RegjistroFaturen(props) {
  ...
  useEffect(() => {
    if (getID) {
      const vendosTeDhenat = async () => {
        try {
          setTeDhenat(data.shfaqPerdoruesit.find((item) => item.perdoruesi.aspNetUserID == getID));
        } catch (err) {
          console.log(err);
        } finally {
          setLoading(false);
  ...
  useEffect(() => {
    if (props.idKalkulimitEdit != 0) {
      const vendosTeDhenat = async () => {
        try {
          const teDhenatKalkulimit = data.shfaqTeDhenatKalkulimit.filter((item) => item.idRegjistrimit == props.idKalkulimitEdit);

          const teDhenatFatures = data.shfaqRegjistrimetNgaID36;
          ...
          setTeDhenatFatures(teDhenatFatures);
        } catch (err) {
          console.log(err);
        } finally {
          setLoading(false);
  ...
  async function handleEdit(id) {
    const p = data.shfaqTeDhenatKalkulimit.filter((item) => item.id == id);
        ...
        setOptionsSelected(
          options.filter((item) => item.value == p[0]?.idProduktit)
        );

        setEdito(true);
        setProduktiID(p[0]?.idProduktit);
        setSasia(p[0]?.sasiaStokut);
        setQmimiBleres(-p[0]?.qmimiBleres);
        setRabati(p[0]?.rabati3);
        console.log(
          options.filter((item) => item.value == p[0]?.idProduktit)
        );
  }
  ...
  useEffect(() => {
        ...
        const fetchedoptions = data.ProduktetPerKalkulim.filter((item) => item.qmimiProduktit > 0).map((item) => ({
