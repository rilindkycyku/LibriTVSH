...
function KartelaFinanciare(props) {
  ...
  useEffect(() => {
    const kartelaEProduktit = async () => {
      try {
        ...
        const response = data.KartelaFinanciare;

        const kalkulimet = response.kalkulimet || [];
        let saldo = 0;
  ...
  useEffect(() => {
    const fetchedoptions = data.shfaqPartneret
      .filter(
        (item) =>
          item.idPartneri !== 1 &&
          item.idPartneri !== 2 &&
    ...
    setOptionsSelected(fetchedoptions.filter((item) => item.value == 15));
  }, []);
              ...
              <Row>
                ...
                <Col>
                  ...
                  <p>
                    ...
                    {parseFloat(
                      kartelaEProduktit &&
                        kartelaEProduktit.partneri?.kartela?.rabati
                    ).toFixed(2) ?? "-"}
                  </p>
                </Col>
                <Col>
