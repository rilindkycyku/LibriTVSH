...
function KartelaEArtikullit(props) {
  ...
  useEffect(() => {
    const kartelaEProduktit = async () => {
      try {
        const kartela = data.KartelaArtikullit;
        setKartelaEProduktit(kartela);
        setKalkulimetKartelaProduktit(
          kartela?.kalkulimet?.map((p, index) => ({
            ID: p.id,
            "NR.": index + 1,
  ...
  useEffect(() => {
    ...
    setOptionsSelected(fetchedoptions.filter((item) => item.value == 1));
    setproduktiID(1);
  }, []);
