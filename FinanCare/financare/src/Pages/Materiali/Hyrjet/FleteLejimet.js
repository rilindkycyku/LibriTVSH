...
function KalkulimiIMallit(props) {
  ...
  useEffect(() => {
    const shfaqKalkulimet = async () => {
      try {
        setLoading(true);
        const kalkulimet = data.shfaqRegjistrimet.filter(
  ...
  useEffect(() => {
    const vendosPartnerin = async () => {
      try {
        setPartneret(data.shfaqPartneretFurnitore);
      } catch (err) {
        console.log(err);
      }
    };
  ...
  useEffect(() => {
    const vendosNrFaturesMeRradhe = async () => {
      try {
        setNrRendorKalkulimit(parseInt(2));
      } catch (err) {
        console.log(err);
      }
    };
  ...
  useEffect(() => {
        const fetchedoptions = data.shfaqPartneretFurnitore
