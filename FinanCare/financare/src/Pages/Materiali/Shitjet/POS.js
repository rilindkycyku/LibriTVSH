...
function POS(props) {
  ...
  useEffect(() => {
    const vendosTeDhenat = async () => {
      try {
        ...
        setproduktetNeKalkulim(teDhenatKalkulimit);
        setIDPartneri(teDhenatFatures[0].idPartneri);

        if (teDhenatFatures[0]?.bonusKartela != null) {
          setKartelaBleresit(teDhenatFatures[0]?.idBonusKartela);
          setTeDhenatKartelaBleresit(teDhenatFatures[0]?.bonusKartela);
        } else {
          setKartelaBleresit(null);
          setTeDhenatKartelaBleresit(null);
        }

        if (teDhenatKalkulimit && teDhenatKalkulimit.length > 0) {
          setIDProduktiFunditShtuar(teDhenatKalkulimit[0].id);
        }
      } catch (err) {
        console.log(err);
      } finally {
  ...
  useEffect(() => {
    // Replace with your API endpoin
    // Assuming the response data is an array of objects with `value` and `label` properties
    const fetchedOptionsBarkodi = data.ProduktetPerKalkulim.filter(
      (item) => item.qmimiProduktit > 0
    ).map((item) => ({
      value: item.produktiID,
      label:
        item.barkodi + " - " + item.emriProduktit + " - " + item.kodiProduktit,
  ...
  const mbyllFature = async (event) => {
    const r = data.shfaqRegjistrimetNgaID;

    document.getElementById("barkodiSelect-input").focus();
    const datat = {
      invoiceNumber: r?.regjistrimet?.nrFatures,
      date: currentDate,
      salesUsername:
        r?.regjistrimet?.stafiID + " - " + r?.regjistrimet?.username,
      items: [
      ...
      totalWithoutVAT: parseFloat(r?.regjistrimet?.totaliPaTVSH).toFixed(2),
      vat: parseFloat(r?.regjistrimet?.tvsh).toFixed(2),
      rabati: parseFloat(r?.rabati ?? 0).toFixed(2),
    };

    // Generate the invoice
    generateInvoice(datat);
  };
  ...
  async function VendosKartelenBleresit() {
    try {
      const kaKartele = data.shfaqKartelat.filter(
        (item) => item.kodiKartela == kartelaBleresit
      );

      if (kaKartele != null) {
        setRabati2(kaKartele.rabati);
        setIDPartneri(kaKartele.partneriID);
        setTeDhenatKartelaBleresit(kaKartele);
  ...
  async function VendosKartelenFshirjesProduktit() {
    try {
      ...
      if (kaKartele != null) {
        if (kaKartele.llojiKarteles == "Fshirje") {
          document.getElementById("barkodiSelect-input").focus();
