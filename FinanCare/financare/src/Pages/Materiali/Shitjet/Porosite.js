...
function KthimIMallitTeBlere(props) {
  ...
  useEffect(() => {
    const shfaqKalkulimet = async () => {
      try {
        setLoading(true);
        const kthimet = data.shfaqRegjistrimet.filter(
  ...
  useEffect(() => {
    const vendosPartnerin = async () => {
      try {
        setPartneret(data.shfaqParneretBleres);
      } catch (err) {
        console.log(err);
      }
    };
  ...
  useEffect(() => {
        const fetchedoptions = data.shfaqParneretBleres.map((item) => ({
