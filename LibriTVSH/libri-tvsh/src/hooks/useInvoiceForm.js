import { useState } from "react";
  ...
  function useInvoiceForm(state, dispatch, setInvoices, invoices, refs) {
    const {
      furnitoriRef,
      datePickerRef,
      nrFaturesRef,
    ...
    const applyCalculatorValue = () => {
      ...
      try {
        const result = evaluate(calculatorState.calculatorValue); // Use math.evaluate instead of eval
          ...
          setTimeout(() => {
            const nextRef =
              field === "vlPaTvsh"
                ...
                : addButtonRef;
            if (nextRef.current) {
              console.log(`Focusing ${field === "vlPaTvsh" ? "TVSH 18%" : field === "tvsh18" ? "TVSH 8%" : "Shtoni Faturen"}`);
              nextRef.current.focus();
            } else {
